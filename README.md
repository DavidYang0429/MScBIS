...
- 历史评论仍从 `AvalonC/MScBIS` 只读展示；新的提问、纠错与经验补充写入 `DavidYang0429/MScBIS`，不转移或重新署名旧评论。
...
本仓库还增加了编辑型首页、双评论区和企业资格检测（当前未启用）等自定义 VuePress 插件。完整的升级原因、Breaking Changes、验证证据、暂缓项和回退方式统一记录在 [依赖维护记录](./DEPENDENCY_MAINTENANCE.md) 中。
